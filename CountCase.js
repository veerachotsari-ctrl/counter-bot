...
const COL_INDEX = { C: 2, D: 3, E: 4, F: 5, G: 6 };
...
function loadConfig() {
    try {
        ...
        CONFIG = { SPREADSHEET_ID: '', SHEET_NAME: 'Sheet1', CHANNEL_IDS: [], BATCH_DELAY: 150, UPDATE_DELAY: 50 };
    }
}
function saveConfig() {
...
// ---------------------------------------------------------
// 2. GOOGLE SHEET FUNCTIONS (คงเดิม)
// ---------------------------------------------------------
        ...
        if (rowIndex >= 0) {
            ...
            for (let i = 0; i < COUNT_COLS; i++) {
                ...
                if (batchCounts[i] > 0) {
                    ...
                    updates.push({ range: `${CONFIG.SHEET_NAME}!${String.fromCharCode(65 + colIndex)}${sheetRowIndex}`, values: [[newCount]] });
                }
            }
        } else {
...
// ---------------------------------------------------------
// 3. DISCORD MESSAGE PROCESSING (คงเดิม)
// ---------------------------------------------------------
async function getUserInfo(client, guild, id, userCache) {
    ...
    } catch { displayName = `UnknownUser_${id}`; username = `unknown_${id}`; }
...
async function processOldMessages(client, interaction, channelId, channelIndex, totalProcessedPerChannel) {
    ...
    while (true) {
        ...
        totalProcessedPerChannel[channelIndex] = `⏳ ช่อง: **#${channel.name}**\n> ประมวลผล: **${processedCount}** ข้อความ`;
        await interaction.editReply({ content: totalProcessedPerChannel.join('\n'), components: [] }).catch(() => {});
        lastId = messages.last().id;
        await new Promise((r) => setTimeout(r, CONFIG.BATCH_DELAY));
    }
    totalProcessedPerChannel[channelIndex] = `🎉 **#${channel.name}** เสร็จสมบูรณ์: **${processedCount}** ข้อความ`;
    await interaction.editReply({ content: totalProcessedPerChannel.join('\n') }).catch(() => {});
}

// ---------------------------------------------------------
// 4. MODULE INITIALIZATION (เพิ่ม /gocc ที่นี่)
// ---------------------------------------------------------
    ...
    client.once(Events.ClientReady, async () => {
        try {
            ...
            if (commandChannel && commandChannel.isTextBased()) {
                ...
                const existing = messages.find(m => m.components[0]?.components.some(c => c.customId === COUNT_BUTTON_ID));
                if (existing) await existing.edit(getStartCountMessage());
                else await commandChannel.send(getStartCountMessage());
            }
        } catch (error) {}
    });

    client.on(Events.InteractionCreate, async (interaction) => {
        // ✨ เพิ่มคำสั่ง /gocc เพื่อเรียก UI กลับมา
        if (interaction.isChatInputCommand() && interaction.commandName === "gocc") {
            return await interaction.reply(getStartCountMessage());
        }

        if (interaction.isButton() && interaction.customId === COUNT_BUTTON_ID) {
            try {
                await interaction.deferReply({ flags: MessageFlags.Ephemeral });
                const activeChannelIds = CONFIG.CHANNEL_IDS.slice(0, 4);
                ...
                await interaction.editReply({ content: `🎉 **การนับเสร็จสมบูรณ์!**\n\n${totalProcessedPerChannel.join('\n')}` });
        ...
        if (interaction.isButton() && interaction.customId === CONFIG_BUTTON_ID) {
            ...
            modal.addComponents(
                new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('spreadsheet_id_input').setLabel('Google Spreadsheet ID').setStyle(TextInputStyle.Short).setValue(CONFIG.SPREADSHEET_ID || '')),
                new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('sheet_name_input').setLabel('ชื่อชีต (Sheet Name)').setStyle(TextInputStyle.Short).setValue(CONFIG.SHEET_NAME || '')),
                new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('channel_list_input').setLabel(`Channel IDs (คั่นด้วย ,) 4 ช่อง`).setStyle(TextInputStyle.Paragraph).setValue(CONFIG.CHANNEL_IDS?.join(', ') || '')),
                new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('batch_delay_input').setLabel('Batch Delay (ms)').setStyle(TextInputStyle.Short).setValue(CONFIG.BATCH_DELAY?.toString() || '150'))
            );
        ...
        if (interaction.isModalSubmit() && interaction.customId === CONFIG_MODAL_ID) {
            ...
            CONFIG.CHANNEL_IDS = interaction.fields.getTextInputValue('channel_list_input').split(',').map(id => id.trim()).filter(id => id.length > 10).slice(0, 4);
